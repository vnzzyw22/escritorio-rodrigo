...
> **⚠ Projeto adaptado para outro cliente em 2026-09-22** (branch `vasconcelos`, não publicado): era o
> site-conceito da Advocacia Rodrigo Alan Dias; o cliente não seguiu adiante, e a estrutura, os componentes
> e os vídeos foram reaproveitados para Vasconcelos — Escritório de Advocacia. As seções abaixo (1 a 19)
> descrevem a direção **original, do Rodrigo** — histórico, não o estado atual do site. O que mudou na
> adaptação (paleta, logo, conteúdo) está registrado na última entrada do Registro de decisões, no fim
> deste arquivo. Contexto de produto atualizado em `PRODUCT.md`; pendências do Vasconcelos em `PENDENCIAS.md`.
...
- 2026-09-21 — Logo oficial no hero. Arquivo recebido: JPEG 1264×842 com o tabuleiro de transparência gravado nos pixels (não é PNG/SVG real). Original preservado, byte a byte, em `assets-originais/logo/logo-oficial-original.jpeg` (fora do git). Derivados em `public/images/logo/`: `rodrigo-alan-dias-symbol.png` (471×481) e `rodrigo-alan-dias-full.png` (606×600), na cor medida no arquivo (rgb 59,59,58), e `rodrigo-alan-dias-symbol-light.png`, a mesma arte monocromática em `paper` para fundo escuro. Só o canal alfa foi calculado (distância até a cor da tinta); nenhum traço foi redesenhado. Os nomes propositalmente não coincidem com os slots `rodrigo-alan-dias` e `rodrigo-symbol`, então o menu, o rodapé e o R do statement continuam como antes. Hero: símbolo pequeno no canto superior esquerdo (celular: acima do título, sobre o fundo escuro), entrada de 0,9 s a partir de 0,5 s, saída ligada à rolagem, título a partir de 1,5 s. O R tipográfico provisório do hero foi retirado (competia com o símbolo real). Falta um vetor para o R grande do statement e do rodapé.
- 2026-09-21 — Símbolo do hero desenhado a traço (`src/components/hero-mark.tsx`, classes `.lg*` em globals.css). Os 19 contornos vêm da vetorização do símbolo oficial (IoU 0,98 contra a máscara do arquivo; tolerância 0,32 px; sem redesenho): anel+R (dois contornos e um preenchimento par-ímpar) e 17 folhas. Estado-base do CSS = final (preenchido, traço invisível); a animação só roda com `prefers-reduced-motion: no-preference`. Linha do tempo: anel e R traçados de 0,25 s a ~1,95 s; folhas de cima para baixo (0,055 s de intervalo, 0,55 s cada); preenchimento de cada peça começa quando o traço dela acaba (0,7 s); o traço some por último, para o resultado final ser exatamente o contorno vetorizado. Título do hero passou a entrar a partir de 1,7 s (lead 2,4 s, botões 2,6 s, faixa 2,85 s). O PNG `rodrigo-alan-dias-symbol-light.png` deixou de ser usado pelo hero (o slot `heroMark` foi removido) e fica como asset.
- 2026-09-21 — Revisão geral do celular. (1) Espaçamento: `--section-y` cai de 96 para 64 px abaixo de md (desktop igual); vãos título→conteúdo de 48–64 px viraram 32–40 px; o Statement perdeu o min-h de 78svh; a faixa de avaliações e o rodapé perderam ~16 px. (2) Fotos: nenhuma passa da margem de 20 px (o retrato deixou de sangrar); o `Frame` ganhou `mobileRatio` (só abaixo de md): Escritório 5:4 (a foto é retrato e o quadro 3:2 cortava 72% dela), Contato 4:3. (3) Hero no celular ocupa exatamente uma tela (mídia flexível de no mínimo 300 px, símbolo, título, texto e botões; a faixa com o nome saiu, repetia o cabeçalho). O vídeo vertical do usuário entra no slot `heroVideoMobile` (9:16). (4) Áreas no celular/tablet (abaixo de lg): o acordeão saiu; cada área é um cartão de foto (texto no alto, 64svh, máx. 520 px) que gruda sob o menu e é coberto pelo próximo; o coberto recua (escala 0,94) e escurece, guiado pela rolagem (CSS scroll-driven, só transform e opacidade). O texto fica no alto porque o cartão seguinte cobre de baixo para cima; há um respiro de leitura entre cartões. Com movimento reduzido ou sem suporte a scroll-driven, os cartões ficam em coluna e sem grudar. Desktop inalterado (índice + painel com troca por hover).
- 2026-09-21 — Seção "Primeira conversa" no lugar de "Perspectivas sobre o Direito" (desligada por `SITE.flags.articles`). Motivo: os artigos não existem, e títulos sem página de artigo, com datas de publicações que não aconteceram, enganavam o visitante. Nova seção (`first-talk.tsx` no servidor; `first-talk-tabs.tsx` no cliente): fundo `paper-deep`, título à esquerda e, à direita, abas por assunto (Trabalho, Contratos, Consumidor, Saúde, Patrimônio; seguem `AREAS`) com uma lista de documentos que o visitante pode marcar; o botão abre o WhatsApp já com o assunto e o que foi marcado. Só orientação prática (sem conselho jurídico, prazo ou promessa) e sem pedir documentos por mensagem. Abas com teclado (setas, Home, End) e, no celular, uma fileira que rola e mantém a aba escolhida à vista. As listas são rascunho para o Rodrigo validar. O item de menu "Conteúdo" saiu (NAV reage à chave `articles`). Os carrosséis e as abas não usam rotação automática.
- 2026-09-21 — Novo material do hero (do usuário): vídeo e foto 16:9 para o desktop e vídeo e foto 9:16 para o celular, cena de livro e balança sobre mesa de madeira, com janela ao entardecer. Processados com ffmpeg: sem áudio, H.264, faststart, laço de 7 s (do original de 8 s: 1–7 s seguido de um dissolve de 1 s entre o fim e o começo, então o laço fecha sem corte seco). Posters: WebP q82 (2752×1536 e 1536×2752). No celular a mídia ganhou altura mínima de 46svh e `object-position` de 50% 88% para enquadrar livro e balança na faixa visível; no desktop o título e o texto ficam sobre a base da imagem e cobrem parte do livro (legíveis, com o degradê funcional). A cena repete a balança, que o briefing original pedia para evitar como elemento decorativo; a escolha é do usuário.
- 2026-09-22 — **Adaptação para Vasconcelos — Escritório de Advocacia** (branch `vasconcelos`, não publicado). O cliente anterior (Rodrigo Alan Dias) não seguiu com o projeto; a pedido do usuário, a estrutura, os componentes, as animações e os vídeos foram reaproveitados para um novo cliente, sem reconstruir do zero. Registro do que mudou:
  - **Dados removidos, nada inventado no lugar:** telefone, endereço, OAB, as 4 avaliações reais (depoimentos de clientes do Rodrigo) e o retrato dele saíram de `site.ts`/`content.ts` e de `public/`. Onde falta fato do Vasconcelos, o site mostra `[X A CONFIRMAR]` (mesmo padrão `pending()` do projeto). O telefone é o caso mais sensível: sem número real, os botões de contato (`wa.me`, `tel:`) usam um número claramente inválido (nunca o do cliente anterior), para não rotear uma mensagem real por engano.
  - **Logo e símbolo (§8, §13):** o círculo+R+louro do Rodrigo é a identidade real de outro escritório e foi removido de tudo o que é publicado (hero, favicon `icon.png`/`apple-icon.png`, imagem de compartilhamento `opengraph-image.png`) — arquivos apagados de `public/images/logo/`. Sem logo confirmada do Vasconcelos, o hero passou a usar `<Wordmark />` (o mesmo lockup tipográfico do cabeçalho: "VASCONCELOS" + "ESCRITÓRIO DE ADVOCACIA") no lugar do símbolo animado a traço; `hero-mark.tsx` (a vetorização do símbolo do Rodrigo) ficou no repositório, mas sem uso, com aviso no topo do arquivo. O "R" grande e quase invisível do Statement e do rodapé (`RMark`) passou a ser dinâmico: a inicial de `SITE.person` (hoje "V"), não mais fixo. `icon.png`/`apple-icon.png`/`opengraph-image.png` foram regerados por *screenshot* de uma rota temporária estilizada com os tokens e a fonte reais do projeto (Libre Caslon Display), depois apagada — método preferível a desenhar um PNG à mão, porque usa a fonte de verdade.
  - **Paleta (§3):** mesma estrutura de tokens (`--ink`, `--graphite`, `--paper`, `--bronze` etc.), só os valores de cor mudaram — matiz neutro girado para um café/terra discreto, luminância de cada token preservada perto do valor anterior (o contraste já testado não deveria mudar muito). `--bronze`/`--bronze-light` mantidos quase iguais: já eram um marrom-bronze discreto, servindo bem ao pedido de "acento marrom médio/terroso" sem precisar mudar. Novos valores:

    | Token | Antes | Agora |
    |---|---|---|
    | `--ink` | #0e0e0d | #16110d |
    | `--graphite` | #1d1e1c | #251c15 |
    | `--graphite-line` | #33342f | #3d2f23 |
    | `--stone` | #8e897f | #9c8d78 |
    | `--ash` | #5d594f | #6b5a46 |
    | `--paper` | #f1ede4 | #f5ede0 |
    | `--paper-deep` | #e7e1d4 | #ebe0c9 |
    | `--paper-line` | #cfc8b8 | #d9cbb0 |
    | `--bronze` / `--bronze-light` | #8f7248 / #b79b6b | inalterados |

  - **Áreas de atuação (§6, tabela do painel):** as cinco áreas de exemplo do Rodrigo viraram três áreas reais do Vasconcelos (Administrativo, Bancário, Imobiliário), com fotos e descrições próprias, escritas só a partir dos pontos do briefing. Três das cinco fotos antigas foram reaproveitadas (renomeadas: `area-01`→`administrativo.webp`, `area-02`→`bancario.webp`, `area-05`→`imobiliario.webp`); as outras duas (`area-03`, `area-04`) ficaram de fora por terem texto errado gravado na imagem ("BANK" numa fachada; placa "DEFESA MÉDICA") e continuam no repositório, sem uso.
  - **Seção "Primeira conversa" (adicionada em 2026-09-21, antes desta adaptação):** as listas de documentos, antes ligadas às áreas do Rodrigo, foram reescritas para as três áreas do Vasconcelos.
  - **Profissional:** nome "Michael Vasconcelos"; formação, OAB e apresentação voltaram a `[A CONFIRMAR]`; o retrato do Rodrigo foi removido (`rodrigo-01.webp` apagado; o slot `lawyer` agora espera `public/images/lawyer/retrato.webp`, mostrando a moldura "Foto a inserir" enquanto não chegar).
  - **Foto de detalhe do escritório:** `office-02` tinha sido trocada, num engano da sessão anterior, por um retrato real do Rodrigo (`rodrigo-02.png`); restaurada para a foto genérica de café e caderno que havia antes.
  - **Vídeo e foto do hero: sem alteração**, a pedido explícito do usuário (mesma cena de livro "Direito Civil Brasileiro" e balança).
  - **Verificado** no navegador (Chrome DevTools MCP), desktop 1440 px e celular 390 px, sem rolagem horizontal: hero, Escritório, Áreas (hover no desktop e cartões empilhados no celular), Profissional (moldura de retrato vazia), Primeira conversa (3 abas), avaliações (só nota, sem citação), Contato, rodapé e `/privacidade`. `tsc`, `eslint` e `next build` limpos.
  - **Não publicado.** O repositório GitHub é público e a Vercel está conectada a ele (qualquer `git push` na `main` publica sozinho); por isso o trabalho ficou num branch à parte (`vasconcelos`), sem *merge* nem *push*. Decisão de onde publicar (repositório novo vs. reaproveitar o atual) fica com o usuário — ver `PENDENCIAS.md` §0.
